...
public function buscarMateriasPrimas($codigo = '', $fecha = '') {
    ...
    public function actualizarMateriaPrima() {
        ...
        // Solo actualizar la fecha si fue proporcionada
        if (!empty($this->fecha_ingreso)) {
            $query .= ", fecha_ingreso = :fecha_ingreso";
        }
    
        $query .= " WHERE id_ingreso = :id";
    
        $stmt = $this->conn->prepare($query);
        
        // Asegúrate de enlazar correctamente el valor de `codigo_materia_prima`
        $stmt->bindParam(":codigo_materia_prima", $this->codigo_materia_prima);
        ...
        // Si se proporcionó la fecha, también enlázala
        if (!empty($this->fecha_ingreso)) {
    ...
    public function eliminarMateriaPrima($id) {
        $query = "DELETE FROM IngresoMateriaPrima WHERE id_ingreso = :id";  // Cambiado id_materia_prima por id_ingreso
        $stmt = $this->conn->prepare($query);
        $stmt->bindParam(":id", $id, PDO::PARAM_INT);
